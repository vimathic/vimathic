...
- **Real mathematics, not pre-rendered animation.** The height of every vertex, the colour of every fragment, and the acceleration of every camera movement is computed from audio analysis and mathematical functions at frame rate — nothing is pre-baked. The engine evaluates canonical mathematical expressions per frame on both of its paths: modular forms (Δ(τ), θ₃(q), η(τ), j(τ)) in the GPU shaders, Bessel functions and Gray-Scott reaction-diffusion in the CPU math worker.
